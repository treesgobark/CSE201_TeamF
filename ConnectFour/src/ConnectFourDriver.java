...
//		int canEdge = -1;
		...
		int firstY = -1;
		int opponentCount = 0;
		if (board.getIconCount() == 1) opponentCount = 2;
		else if (board.getIconCount() == 2) opponentCount = 1;
		...
		for (int x = 0; x < board.getButtons().length; x++) {
			...
			if (yIsSafe) {
				...
				int temp = board.getIconColor()[x][y];
...
//		barLabel.setText("Red's wins: " + redWinCount + ". Yellow's Wins: " + yellowWinCount + ". Ties: " + tieCount + ".");
	...
	public void checkForResolution() {
		if (checkForWin(0)) {
			if (winner == 1) {
//				redWinCount++;
				JOptionPane.showMessageDialog(board, "Red Wins!");
				resetBoard();
			}
			if (winner == 2) {
//				yellowWinCount++;
				JOptionPane.showMessageDialog(board, "Yellow Wins!");
				resetBoard();
			}
		} else if (checkForTie()) {
//			tieCount++;
			JOptionPane.showMessageDialog(board, "It's a tie because you both suck");
			resetBoard();
		}
//		barLabel.setText("Red's wins: " + redWinCount + ". Yellow's Wins: " + yellowWinCount + ". Ties: " + tieCount + ".");
	}
	...
	public boolean checkForWin(int pos) {
		...
		// System.out.print(pos + ": " + x + ", " + y + ": " + board.getIconColor()x][y]);
	...
	public boolean checkForGap(int x, int y, int inARow) {
		...
		if (x < board.getButtons().length) {
			...
			if (inARow == 2) {
				...
				if (board.getIconCount() == 1)
					if (board.getIconColor()[x][y] != 1) {
						// System.out.println("Passed the third check");
						if (checkForGap(x + 1, y, inARow + 1))
							return true;
					}

				if (board.getIconCount() == 2)
					if (board.getIconColor()[x][y] != 2) {
						// System.out.println("Passed the third check");
						if (checkForGap(x + 1, y, inARow + 1))
							return true;
					}
